  ...
  public void processElement(@Element KV<String, String> inputJSON, ProcessContext processContext) {
    ...
    try {
      ...
      processContext.output(
          CommonConstants.SUCCESS_TAG, KV.of(logMessage.getLogType(), inputJSON.getValue()));
    } catch (Exception e) {
      FailureMetaData failureMetaData =
          CommonUtil.getDataValidationFailureResponse(
              JSONParser.class.toString(), e.getMessage(), inputJSON.getValue());
      processContext.output(
          CommonConstants.FAILURE_TAG, KV.of(inputJSON.getKey(), failureMetaData));
    }
  }
}
